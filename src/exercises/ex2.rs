...
fn is_sorted_rec(arr: &Vec<i32>, pos: usize) -> bool {
    ...
    match max_rec(arr, pos + 1) {
        Some(maximum) => arr[pos] <= maximum && is_sorted_rec(arr, pos + 1),
        None => true,
    }
}
...
fn max_index(arr: &Vec<i32>, k: usize) -> usize {
    // 0..k exclusive (assume not empty & k>=0)
    let mut index = 0;
    let mut max = arr[0];
    for i in 0..k {
        if arr[i] >= max {
