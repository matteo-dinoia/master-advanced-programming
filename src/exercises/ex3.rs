...
pub(crate) fn test_all() {
    println!(
        "{} {} {} {}",
        is_it_luhn("ciao".to_string()),
        is_it_luhn("134".to_string()),
        is_it_luhn("0".to_string()),
...
mod test2 {
    ...
    pub fn magic_sentence(
        hashmap: &HashMap<i32, Sentence>,
        i: i32,
        j: i32,
    ) -> Result<Sentence, &str> {
    ...
    impl Line {
        ...
        pub fn contains(&self, p: &Point) -> Result<(), String> {
            if p.x < self.start.x || p.x > self.end.x {
                ...
                Err("Not in line (not on infinite line)".to_string())
            }
        }
    }
}
...
impl Display for BoxShipping {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Shipment of {} (barcode: {}) in date {:02}/{:02}/{:04} at time {:02}:{:02}",
            self.name,
            self.barcode,
            self.shipment_date.0,
            self.shipment_date.1,
            self.shipment_date.2,
            self.shipment_hour.0,
            self.shipment_hour.1
        );
...
impl VendingMachine {
    ...
    pub fn insert_coin(&mut self, coin: Coin) -> Result<&str, &str> {
        self.coins += match coin {
            ...
            Coin::Eur2 => 200,
            Coin::TooLow => {
                return Err("Cannot accept 0.01 or 0.02");
            }
            Coin::Undetected => {
                return Err("Could not verify coin");
            }
        };
    ...
    pub fn buy(&mut self, item: Item) -> Result<u32, &str> {
        if item.cost > self.coins {
            return Err("Not enought money");
        }

        let mut quantity = self.items.get_mut(&item).ok_or("Item not present")?;

        if *quantity == 0 {
            return Err("The requested item is out of stock");
        }
        *quantity -= 1;
...
impl Parking {
    ...
    pub fn add(&mut self, plate: String, owner: String, cost: u32) -> bool {
        self.plate_data.insert(plate, Car { owner, cost }).is_some()
    }
